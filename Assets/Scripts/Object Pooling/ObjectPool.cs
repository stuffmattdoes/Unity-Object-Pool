...
public class ObjectPool : MonoBehaviour {
	...
	public string containerPrefix = "_Pool_";
	...
	void Start() {
		...
		for (int i = 0; i < Entries.Length; i ++) {
			...
			if (Entries[i] != null) {
				...
				ContainerObject[i] = new GameObject (containerPrefix + Entries[i].Prefab.name);
...
//			obj.transform.parent = ContainerObject[i].transform;
	...
	public GameObject GetPooledObject(GameObject pooledObj) {
		...
		for (int i = 0; i < AvailablePool.Count; i++) {
			...
			if (pooledObj.name != AvailablePool[i].name) {
				continue;
			}
		...
		if (GrowIfUnavailable) {
			...
			GameObject newObj = Instantiate (pooledObj) as GameObject;
			newObj.name = pooledObj.name;
			PoolObject(newObj);
			return newObj;
		}
